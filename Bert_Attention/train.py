...
def do_train(df_train):
    ...
    label = [str(l) for l in df_train['label']]
    for fold, (trn_idx, val_idx) in enumerate(skf.split(df_train['text'], label), 1):
        print(f'Fold {fold}')
        # print(trn_idx)
        # print(type(trn_idx))
        trn_idx = [int(idx) for idx in trn_idx]
...
if __name__ == '__main__':
    df_train = pd.read_csv('data/toutiao_train.csv', sep=',')
